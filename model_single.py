...
class Group_vit(nn.Module):
    def __init__(self, *, image_size, patch_size, num_classes, dim, depth_T, depth_G, heads, mlp_dim, vocab_size, channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0.):
        ...
        self.proj_text = EasyFF(in_feature=1024,out_feature=dim,dropout=dropout) # 512 for the clip word tokenization
    ...
    def forward(self, x, text):
        ...
        x_img = self.proj_img(x)
        x_text = self.proj_text(x[:,:-1,:]) # get the first 576
        bs, n, _ = x_img.shape
        x_img += self.pos_embedding
        ...
        y_tmp = rearrange(x_text, 'b (h n) d -> b h n d', h=16) # [b, 36, 1600]
        y = torch.mean(y_tmp, dim=1).squeeze()
        xy = torch.cat((x_img,y), dim=1)
